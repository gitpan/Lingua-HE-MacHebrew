...
print 1
   && "" eq encodeMacHebrew("")
   && "" eq decodeMacHebrew("")
   && "Perl" eq encodeMacHebrew("Perl")
   && "Perl" eq decodeMacHebrew("Perl")
   ? "ok" : "not ok", " ", ++$loaded, "\n";
...
print 1
   && "\0" eq encodeMacHebrew("\0")
   && "\0" eq decodeMacHebrew("\0")
   && "\cA" eq encodeMacHebrew("\cA")
   && "\cA" eq decodeMacHebrew("\cA")
   && "\t" eq encodeMacHebrew("\t")
   && "\t" eq decodeMacHebrew("\t")
   && "\x7F" eq encodeMacHebrew("\x7F")
   && "\x7F" eq decodeMacHebrew("\x7F")
   ? "ok" : "not ok", " ", ++$loaded, "\n";
...
print 1
   && "\r" eq encodeMacHebrew("\r")
   && "\r" eq decodeMacHebrew("\r")
   ? "ok" : "not ok", " ", ++$loaded, "\n";

print 1
   && "0123456789" eq encodeMacHebrew("0123456789")
   && "0123456789" eq decodeMacHebrew("0123456789")
   ? "ok" : "not ok", " ", ++$loaded, "\n";
...
print 1
   && $uniDigitRL eq decodeMacHebrew($macDigitRL)
   && $macDigitRL eq encodeMacHebrew($uniDigitRL)
   ? "ok" : "not ok", " ", ++$loaded, "\n";
...
for ($char = 0; $char <= 255; $char++) {
    $bchar = chr $char;
    $uchar = encodeMacHebrew(decodeMacHebrew($bchar));
...
print "\x{C4}" eq decodeMacHebrew("\x{80}")
   && "\x{C4}" eq decodeMacHebrew(pack 'U', 0x80)
   ? "ok" : "not ok", " ", ++$loaded, "\n";

print "\x30" eq encodeMacHebrew("\x30")
   && "\x30" eq encodeMacHebrew("\x{202D}\x30") # with LRO
   && "\xB0" eq encodeMacHebrew("\x{202E}\x30") # with RLO
   ? "ok" : "not ok", " ", ++$loaded, "\n";

print "\x39" eq encodeMacHebrew("\x39")
   && "\x39" eq encodeMacHebrew("\x{202D}\x39") # with LRO
   && "\xB9" eq encodeMacHebrew("\x{202E}\x39") # with RLO
   ? "ok" : "not ok", " ", ++$loaded, "\n";
